...
class AICommands(commands.Cog):
    ...
    async def junkstats(self, interaction: discord.Interaction) -> None:
        ...
        metrics = [
            ...
            "Bus travel miles logged", "Post-season plate appearances", "Spring Training wild pitches",
            "Games played while the price of gas was low", "Ejections by a home plate umpire",
            "Games played in ZIP codes starting with an even number", "Distance from the nearest Post Office",
            "Elevation above sea level", "Proximity to a federally funded bridge", "Local subway access",

            # --- PITCHING & COMMAND ---
            "Innings Pitched (IP)", "Balks", "Wild Pitches", "Pickoff Attempts", "Successful Pickoffs",
            "Hit Batsmen (HBP)", "Uncaught Third Strikes", "Pitch Clock Violations", "Mound Visits",
            "Holds", "Blown Saves", "Inherited Runners Stranded", "Complete Games", "Shutouts",
            "Earned Runs allowed on Tuesdays", "Intentional Walks issued", "Warm-up pitches thrown",

            # --- BATTING & BASERUNNING ---
            "Intentional Walks drawn", "Caught Stealing", "Stolen Base Percentage on Turf",
            "Sacrifice Bunts", "Sacrifice Flies", "Infield Singles", "Bunt Hits", "Grounded into Double Plays (GIDP)",
            "Strikeouts on a Full Count", "Pinch-Hit Plate Appearances", "Triples west of the Mississippi",
            "Home Runs hit in Dome Stadiums", "RBIs on a Friday", "Total Bases in Doubleheaders",
            "Lead-off walks", "Plate appearances ending in a foul-out",

            # --- DEFENSE & FIELDING ---
            "Range Factor", "Defensive Runs Saved (DRS)", "Total Zone Runs", "Putouts at Shortstop",
            "Assists by a Left-fielder", "Errors committed on artificial turf", "Double Plays Turned",
            "Outfield Assists", "Passed Balls", "Catcher's Interference", "Putouts at Third Base",
            "Errors in day games", "Putouts in extra innings", "Infield Fly Rule calls",

            # --- ADVANCED & SABERMETRIC ---
            "WAR (Wins Above Replacement)", "OPS+", "ERA+", "Win Shares", "FIP (Fielder Independent Pitching)",
            "Win Probability Added (WPA)", "Secondary Average", "Isolated Power (ISO)",
            "Batting Average on Balls in Play (BABIP)", "Exit Velocity on groundouts",

            # --- TEAM, LEAGUE & MARGINS ---
            "Wins by exactly x run", "Losses by exactly x runs", "Extra-inning losses",
            "Interleague wins on a Tuesday", "Division-lead days", "Games behind the leader",
            "Doubleheaders swept", "Series sweeps in April", "Wins against left-handed starters",
            "One-run victories in dome stadiums", "Winning streaks during the second half"

            # --- TRUELY OBSCURE ---
            "Ground ball-to-fly ball ratio (GB/FB)", "Opponent Batting Average on a 3-1 count",
            "Average distance of fly-ball outs", "Infield fly rule call frequency",
            "Pitcher assists on ground balls to the first base side", "Pickoff attempts on runners with lead-off walks",
            "Games played while the Dow Jones finished lower than its opening", "Total bases on hits to the opposite field",
            "Errors committed while the sun was at a specific angle (5:00 PM starts)", "Successful bunts against southpaws",
            "Wins by exactly seven runs", "Extra-inning walks allowed by a closer",
            "Total bases recorded in cities with a name containing the letter 'Z'", "Distance from the stadium to the nearest state line",
